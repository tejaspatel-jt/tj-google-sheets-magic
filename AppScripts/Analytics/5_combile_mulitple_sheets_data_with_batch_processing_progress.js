...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('📈Show Batch Combine Progress', 'showBatchCombineProgress')
    .addToUi();
}
...
function batchCombineSheets() {
  ...
  var inProcessSheetNames = sheetsToProcess.map(function(sheet) { return sheet.getName(); }).join(', ');
  ss.toast(
    'Batch combining data...\nSheets being processed: ' + inProcessSheetNames + '\nYou will be notified once this batch is done!',
    '⚠️ Attention ⚠️',
    -1
  );
  ...
  if (end >= validSheets.length) {
    ...
    ui.alert(
      "❓ Batch processed/unprocessed Info ℹ️",
...
// 5. Show Batch Combine Progress
function showBatchCombineProgress() {
  ...
  if (!outSheet) {
    ...
    // CombinedData sheet is missing, so consider all as unprocessed
    var unprocessedSheets = validSheets.map(function(sheet) {
      return sheet.getName() + " ⭕";
    });
    ui.alert(
      "📈 Batch Process Status 📈",
      '⚠️ "CombinedData" sheet not found!\n\n' +
      'All sheets are considered unprocessed.\n\n' +
      'Unprocessed sheets:\n' + unprocessedSheets.join('\n'),
      ui.ButtonSet.OK
    );
    return;
  ...
  var lastProcessedIndex = Number(scriptProperties.getProperty('lastProcessedIndex')) || 0;
  var end = Math.min(lastProcessedIndex, validSheets.length);
